...
const COLUMNS: { key: string; header: string }[] = [
  ...
  { key: "inventoryInside", header: "Cars inside" },
  { key: "inventoryOutside", header: "Cars outside" },
  { key: "inventoryAgePctOver5", header: "% older than 5 years" },
...
function rowValues(d: Dealership, s: SurveyPayload | undefined, photos: number): Record<string, unknown> {
  return {
    ...
    inventoryInside: s?.inventoryInside ?? "",
    inventoryOutside: s?.inventoryOutside ?? "",
    inventoryAgePctOver5: s?.inventoryAgePctOver5 ?? "",
